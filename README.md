...
Using a fixed amount of bits per integer may be a good idea in some particular cases but it has a big flaw that arises when the maximum number in the array is very big and it'is an outlier compared to the other numbers. To solve this issue we can use variable length integer coders. In particular we use elias-$\gamma$ and elias-$\delta$ codes, developed by [Peter Elias](https://en.wikipedia.org/wiki/Peter_Elias). Why can't we just store the binary representation of our numbers one ofter the other? Because otherwise we would not be able to recognize one integer from the next one (remember that we are just storing a sequence of bits in memory). If I gave you the sequence `S = 011101101001`, telling you that I have encoded some integers in it, would you be able to decode it? It may be `01 110110 1001` or `01 110 110 1001` and many other combinations.
...
The biggest issue of representing integers with $\delta$-code is the time to access a number. You need to do binary search on the support array which is a logarithmic operation, we want to achieve constant time. To solve this problem we can use the **Elias Fano** representation. SDSL gives us `sdsl::bit_vector` which is basically a specialization of `sdsl::int_vector<1>`. I will now explain Elias Fano assuming that we have the following two functions already implemented.
...
Now we just need a way to `access` (that's the name of the function too) a number at a specific index. We assume that we have the `select1` implemented. The following is the implementation of `access`.
...
Know that there's a known issue (bug) that will give you different results compared to the ones i described. This bug also makes the library slower (by a small factor). If you use [other](https://github.com/vgteam/sdsl-lite) libraries this is fixed.
